...
# 
ids, stat = findHDCells(tuning_curves, z = 30, p = 0.001, m=1)
...
interval = nts.IntervalSet(start = wake_ep.loc[1,'start'], 
                           end=wake_ep.loc[1,'start']+10*60e6)
...
for string, objct in zip(['spikes_HDCells', 'tuning_curves','tuning_curves_HDcells',\
    ...
    with open(os.path.join(data_directory_load, string + '.pickle'), 'wb') as handle:
        pickle.dump(objct, handle, protocol=5)